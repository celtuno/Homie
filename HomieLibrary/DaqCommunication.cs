...
namespace HomieLibrary
{
    public class DaqCommunication 
    {
       ...
       /* public void WriteDigitalOut(bool[] Channelstatus, string[] channel)
        ...
        public void WriteDigitalOut(bool[] Channelstatus, string channel)
        {
            ...
            try
            {
                ...
                for (int i = boolArray.Count() - 1; i >= 0; i--)
                {
                    ...
                    digitalValueArray[i] = boolArray[i];//Convert.ToInt32(boolArray[i]);
                    lastValue = digitalValueArray[i];
                }
                
                //Write digital signal out 
                NationalInstruments.DAQmx.Task digitalOutTask = new NationalInstruments.DAQmx.Task();
                digitalOutTask.DOChannels.CreateChannel(channel, //"dev6/Port0/line0:7",
                    "digitalOutChannels",
                    ChannelLineGrouping.OneChannelForAllLines);                
            ...
            try
            {
                ...
                NationalInstruments.DAQmx.Task digitalOutTask = new NationalInstruments.DAQmx.Task();
                    digitalOutTask.DOChannels.CreateChannel(channel,
                        "digitalOutChannels",
                        ChannelLineGrouping.OneChannelForAllLines);
                    //DigitalMultiChannelWriter writer = new
                    //DigitalMultiChannelWriter(digitalOutTask.Stream);
                    //byte[] testArray = {1,1,1,1,1,1,1,1 };
                    //byte [,] test = { {0,0 }, { 1, 1 }, { 2, 1 }, { 3, 1 }, { 4, 1 },{ 5, 1 }, { 6, 1 }, { 7, 1 }};
                    ...
                    //writer.WriteSingleSamplePort(true, testArray);
        ...
        public void DaqToDb(string daqName,  string daqAddress,string daqModel, int houseId)
        {
